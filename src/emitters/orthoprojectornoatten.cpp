...
MI_VARIANT class OrthoProjectorNoAtten final : public Emitter<Float, Spectrum> {
public:
    MI_IMPORT_BASE(Emitter, m_flags, m_to_world, m_needs_sample_3)
    MI_IMPORT_TYPES(Texture)

    OrthoProjectorNoAtten(const Properties &props) : Base(props) {
    ...
    std::string to_string() const override {
        ...
        oss << "OrthoProjectorNoAtten[" << std::endl
...
MI_IMPLEMENT_CLASS_VARIANT(OrthoProjectorNoAtten, Emitter)
MI_EXPORT_PLUGIN(OrthoProjectorNoAtten, "OrthoProjectorNoAtten emitter")
