...
def run_scene_radiance(scene, scene_name, **kwargs):
    ...
    if output_filename is not None and output_path is not None:
        ...
        if os.path.exists(numpy_output_filename) and kwargs.get("exit_if_file_exists", False):
...
def run_scene_velocity(scene, scene_name, **kwargs):
    print("capture time: ", kwargs.get("capture_time", 0.0017))
    ...
    print("Saving result as ", numpy_output_filename)
    np.save(numpy_output_filename, img_velocity)
...
def run_scene_doppler_tof(
    ...
    **kwargs
    ...
    if output_path is not None and expname is not None:
